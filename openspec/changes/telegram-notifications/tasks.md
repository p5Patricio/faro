...
- [x] 2.1 **RED**: write a failing test asserting a rendered message containing `<`, `>`, `&` in an interpolated ticker/reason value is not passed through unescaped to `session.post` (threat matrix: untrusted text in HTML).
- [x] 2.2 **RED**: write a failing test asserting a `FakeSession.post` raising `HTTPError(f"...for url: .../bot{TOKEN}/sendMessage")` never leaks `TOKEN` in the returned dict (threat matrix: credential in the URL path).
